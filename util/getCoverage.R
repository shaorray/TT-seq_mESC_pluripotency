  ...
  out_matrix <- foreach( chr = .seqnames, .combine = rbind ) %dopar%
    {
      ...
      get_new_len <- function(x) {
        x = tmp_cov[ranges(tmp_gr[x])] %>% 
          as.numeric() %>% 
          binSums(bin_width)
      ...
      if (!(bin_width > 0) | is.na(bin_width) | is.null(bin_width)) 
        bin_width = 50 # default
      
      if (!(new_len > 0) | is.na(new_len) | is.null(new_len)) 
        new_len = ceiling(width(intervals[1]) / bin_width)
      
      sapply(seq_along(tmp_gr), get_new_len) %>% t() %>% "/"(bin_width)
    }
...
viewCoverage <- function(bam.file.list,
  ...
  if (is.null(names(bam.file.list))) 
    names(bam.file.list) <- seq_along(bam.file.list)
  if (is.null(bam.sample.names)) 
    bam.sample.names <- names(bam.file.list)
  if (is.null(size.factor.list)) 
    size.factor.list <- lapply(bam.file.list, function(x) rep(1, length(x)))
  if (any(names(size.factor.list) != names(bam.file.list))) 
    names(size.factor.list) <- names(bam.file.list)
  if (is.null(cov_color_list)) 
    cov_color_list <- lapply(bam.file.list, function(x) c("plus"='#bd5734', "minus"='#feb236'))
  ...
  coverage_list <- vector(mode = "list", length = length(bam.file.list))
  names(coverage_list) <- names(bam.file.list)
  
  # Get coverage and combine replicates
  for (sample in names(bam.file.list)) 
  {
    bam_files = bam.file.list[[sample]]
    plus.cov = 
      combine_rep(
    ...
    if (stranded) 
    {
      ...
      if (as.character(strand(interval)) == "-") plus.cov = rev(plus.cov)
      coverage_list[[sample]] = list("plus" = binSums(plus.cov, bin_width = bin_width) / bin_width)
    }
  }
