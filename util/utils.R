...
{ # process kallisto genecode transcriptome outputs 
  # save the main tx for each protein_coding gene and lincRNA 
...
{ # r_squared decomposition of regression through origin
  ...
  keep.idx = !is.na(rowSums(cbind(X, Y))) & is.finite(rowSums(cbind(X, Y)))
  X = X[keep.idx, ]; Y = Y[keep.idx]
  stopifnot(var(Y) > 0 & length(Y) == nrow(X))
  message(paste("Number of observations in use:", sum(keep.idx)))
...
plot_ma <- function(baseMean, log2FC, 
                    cut_baseMean = 0.5,
                    xlab = "baseMean",
                    ylab = "Log2FC",
                    title = "", 
                    p_col = "grey50",
                    pin_point = NA, # index to color
                    pin_color = "deeppink2") {
  dat = data.frame("x" = baseMean, "y" = log2FC) %>% 
  ...
  g <- ggplot(dat, aes(x = x, y = y, color = density)) +
    ...
    annotate("text", x = Inf, y = Inf, 
             label = paste("Log2FC sd =", round(sd(dat$y), 2)),
             vjust = 1.3, hjust = 1.1) + 
    ggtitle(title) +
    theme_setting +
    theme(legend.position = "none")
  
  if (!is.na(pin_point[1])) {
    dat_highlight <- data.frame("x" = baseMean[pin_point], "y" = log2FC[pin_point]) %>% 
      dplyr::filter(complete.cases(.) &
                      x > (cut_baseMean) & 
                      !is.infinite(x) & 
                      !is.infinite(y))
    g + geom_point(data = dat_highlight, aes(x = x, y = y),
                   size = 0.001, color = pin_color)
  } else {
    g
  }
}
