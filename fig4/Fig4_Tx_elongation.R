...
ggsave(plot = grid.arrange(g1, g2, g3, nrow = 1),
       filename = "FigS5_Pause_index_tx_correlation.png",
       path = "../figS5/figs",
       device = "png", width = 10, height = 3.5)
...
ggsave(filename = "Fig4_RNA_synthesis_explained.png", path = "figs",
...
ggplot(dat_speed_cmp2, aes(x = (speed_paper), y = (speed_measured))) +
  ...
  scale_y_continuous(name="Est. speed (kb/min)",
                     breaks= c(0, 1, 2, 3),
                     labels= c(0, 1, 2, 3)) +
  theme_setting 
ggsave(filename = paste0("Fig4_measured_speed_vs_paper_speed.png"), 
       path = "../fig4/figs/",
...
ggsave(plot = grid.arrange(g11, g7, g12, g10, g8, ncol = 2),
       filename = "Fig4_TTseq_Pol2S5p_speed_interplays2.png", path = "figs",
...
saveRDS(dat_speed_change, "../fig4/data/dat_speed_change.RData")
...
ggsave(plot = grid.arrange(g13, g14, ncol = 2),
       filename = "FigS7_MAplot_speed_change.png", 
       path = "../figS7/figs", device = "png", width = 12, height = 6)

# speed changes ~ epigenome features



# epigenome features speed explained

measured_speed <- data.frame(Feature = colnames(feature_data),
                             R2 = multi_variance_explained(feature_data[match(rownames(dat_speed), rownames(feature_data)), ],
                                                           trim_quantile((dat_speed$speed), 0.99), F),
                             Corr = multi_variance_explained(feature_data[match(rownames(dat_speed), rownames(feature_data)), ],
                                                             trim_quantile((dat_speed$speed), 0.99), T) )

est_speed <- data.frame(Feature = colnames(feature_data),
                        R2 = multi_variance_explained(feature_data[match(rownames(dat_speed), rownames(feature_data)), ],
                                                      trim_quantile((dat_speed$v_hat_SL), 0.99), F),
                        Corr = multi_variance_explained(feature_data[match(rownames(dat_speed), rownames(feature_data)), ],
                                                        trim_quantile((dat_speed$v_hat_SL), 0.99), T) )
