...
ggsave(plot = grid.arrange(g0.1, g0.2, g0.3, g0.4, nrow = 2),
       filename = "Fig2_Scatter_halflife_copy_SL.png",
       path = "../fig2/figs", device = "png", width = 8, height = 8)
...
sample_Tx_counts_Rates$location <- "protein coding"
...
dat$location <- factor(dat$location, c("protein coding", "intergenic", "uaRNA", "asRNA", "conRNA", "daRNA", "usRNA", "dsRNA", "gene"))

ggplot(dat[!is.na(dat$Half_life) & 
             dat$location %in% c("protein coding", "intergenic", "uaRNA", "asRNA", "conRNA"), ], 
       aes(x = location, y = log10(Half_life), fill = Sample)) +
  geom_boxplot(outlier.colour = NA) +
  scale_fill_manual(values = colors_20[c(13, 2, 2, 20, 7, 7)]) +
  xlab("") +
  scale_y_continuous(name="Half-life (min)", breaks=0:3, labels=10^(0:3), limits = c(0, 3.5)) +
  theme_setting +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
ggsave(filename = "FigS2_sample_halflife_boxplot.png", path = "../figS2/figs",
...
ggplot(dat[!is.na(dat$Half_life) & 
  ...
  scale_fill_manual(values = colors_20[c(13, 2, 2, 20, 7, 7)]) +
  xlab("") +
  scale_y_continuous(name="Copy", breaks=(-1):2, labels=10^((-1):2), limits = c(-1, 3)) +
  theme_setting +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
ggsave(filename = "FigS2_sample_copy_boxplot.png", path = "../figS2/figs",
       device = "png", width = 5, height = 4)
...
# refer to https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5399622/
...
pluripotent_markers <- list("General pluripotency" = c("Myc", "Mycn", "Nanog", "Pou5f1", "Sox2", "Sall4", "Tdgf1", "Utf1", "Zic2", "Zic3"),
                            ...
                            "Post-implantation" = c("Cdh2", "Dnmt3a", "Dnmt3b", "Etv4", "Etv5", "Fgf5", "Fgf10", "Fgf15", "Fgfr1", "Fzd7",  
                                                    "Id1", "Lefty1", "Lefty2", "Myc", "Otx2", "Sall2", "Sox3", "Sox4", "Sox11", "Pou3f1", "Nodal", "Zic2", "Zic3" ),
...
g1 <- ggplot(dat_markers[dat_markers$Type %in% c("General pluripotency", "Naïve pluripotency"), ], 
  ...
  ylab("Copy / cell") +
  xlab("") +
  scale_y_continuous(breaks = (-1:3), labels = 10^(-1:3), limits = c(-2, 3)) +
...
g2 <- ggplot(dat_markers[dat_markers$Type %in% c("General pluripotency", "Naïve pluripotency"), ], 
             aes(x = gene_symbol, y = log10(Half_life), 
                 color = Sample, group = gene_symbol)) +
  geom_point(cex = 2, shape=21, stroke = 1.5) +
  geom_line(color = "grey50") +
  ylab("Half life / min") +
...
ggsave(plot = grid.arrange(g1, g2, nrow = 1, widths = c(4, 5)),
       filename = "Fig2_pluripotent_markers_copy_half_life_1.png",
       path = "../fig2/figs", device = "png", width = 7, height = 5)

g3 <- ggplot(dat_markers[!dat_markers$Type %in% c("General pluripotency", "Naïve pluripotency"), ], 
             aes(x = gene_symbol, y = log10(Copy), 
                 color = Sample, group = gene_symbol)) +
  geom_point(cex = 2, shape=21, stroke = 1.5) +
  geom_line(color = "grey50") +
...
g4 <- ggplot(dat_markers[!dat_markers$Type %in% c("General pluripotency", "Naïve pluripotency"), ], 
             aes(x = gene_symbol, y = log10(Half_life), 
                 color = Sample, group = gene_symbol)) +
  geom_point(cex = 2, shape=21, stroke = 1.5) +
  geom_line(color = "grey50") +
...
ggsave(plot = grid.arrange(g3, g4, nrow = 1, widths = c(4, 5)),
       filename = "Fig2_pluripotent_markers_copy_half_life_2.png",
       path = "../fig2/figs", device = "png", width = 7, height = 8)
...
ggsave(plot = grid.arrange(grobs = lapply(list(g3.2, g3.1, g3.3), "+", margin),nrow = 1),
       filename = "FigS4_saturation_copy_half_life_mu.png",
       path = "../figS4/figs", device = "png", width = 12, height = 3.5)
...
ggsave(plot = cowplot::plot_grid(g4.1, g4.2, g4.3, g4.4, ncol = 1, align = "v"),
       filename = "Fig2_pluripotent_group_copy_half_life_mu.png",
       path = "../fig2/figs", device = "png", width = 3.5, height = 10)
