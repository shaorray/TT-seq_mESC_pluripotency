...
if (T) {
  ...
  png("figs/FigS2_Spike_in_half_life_tile.png", 500, 500)
  par(mar=c(4,4,1,1))
  ...
  if (T) {
    ...
    with(SampleSpCounts[1:6, ],
         text(x = log2(FRNA) + 0.5,y = log2(LRNA) + 0.8, adj = 0.1,
              labels = gsub("chr(.*)","\\1", SpikeIns), cex = 1.2))
...
if (T) {
  ...
  png("figs/FigS2_Spike_in_copy_number_tile.png", 500, 500)
  par(mar = c(4,4,1,1))
  layout(mat=matrix(c(1, 2), 1, 2),
         widths=c(2, 0.5),
         heights=c(2, 2), TRUE)
...
# --------------------------------------------------------------------------------------- #
sample_Tx_counts <- readRDS('../figS2/data/sample_Tx_counts_Rates_combined2.RData')
sample_Tx_counts$Sample <- factor(sample_Tx_counts$Sample, 
                                  levels = c("SL", "2i_2d", "SL2i_2d", "mTORi_1d", "mTORi_2d"))

g1 <- sample_Tx_counts %>%
  dplyr::filter(!is.na(Half_life) & is.finite(Half_life) & Half_life > 0 & Sample != "2i_7d"& gene_id %in% names(gene.gr)) %>%
  ggplot(aes(x = Sample, y = log10(Half_life), fill = Sample)) +
  geom_boxplot(position = "identity", outlier.size = 0, notch = T) +
  ggpubr::stat_compare_means(label = "p.signif", method = "t.test",
                             ref.group = "SL") +
  scale_y_continuous(name = "Turnover half-life (min)", 
                     breaks = 0:3, labels = 10^(0:3)) +
  scale_fill_manual(values = colors_20[c(13, 2, 10, 20, 7)]) +
  xlab("") + ggtitle("mRNA turnover (n=10537)") +
  theme_setting +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))

g2 <- sample_Tx_counts %>%
  dplyr::filter(!is.na(Copy) & is.finite(Copy) & Copy > 0 & Sample != "2i_7d" & gene_id %in% names(gene.gr)) %>%
  ggplot(aes(x = Sample, y = log10(Copy), fill = Sample)) +
  geom_boxplot(outlier.size = 0, notch = T) +
  ggpubr::stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = "SL") +
  scale_y_continuous(name = expression(paste("Total RNA Copy (", cell^{-1}, ")")) , 
                     breaks = -2:2, labels = 10^(-2:2), limits = c(-3, 4)) +
  scale_fill_manual(values = colors_20[c(13, 2, 10, 20, 7)]) +
  xlab("") + ggtitle("mRNA Copy (n=10673)") +
  theme_setting +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(plot = grid.arrange(g1, g2, nrow=1),
       filename = "Fig1_boxplot_mRNA_half_life_turnover.png",
       device = "png", path = "../fig1/figs", width = 8, height = 4)

sample_Tx_counts <- sample_Tx_counts %>%
  dplyr::filter(!is.na(Half_life) & is.finite(Half_life) & Half_life > 0 & Sample != "2i_7d"& gene_id %in% names(gene.gr)) %>%
  dplyr::filter(!is.na(Copy) & is.finite(Copy) & Copy > 0 & Sample != "2i_7d" & gene_id %in% names(gene.gr))

# p-vals
t.test(log10(sample_Tx_counts$Half_life)[sample_Tx_counts$Sample == "SL"], 
       log10(sample_Tx_counts$Half_life)[sample_Tx_counts$Sample == "2i_2d"])
t.test(log10(sample_Tx_counts$Half_life)[sample_Tx_counts$Sample == "SL"], 
       log10(sample_Tx_counts$Half_life)[sample_Tx_counts$Sample == "mTORi_1d"])
t.test(log10(sample_Tx_counts$Half_life)[sample_Tx_counts$Sample == "SL"], 
       log10(sample_Tx_counts$Half_life)[sample_Tx_counts$Sample == "mTORi_2d"])

t.test(log10(sample_Tx_counts$Copy)[sample_Tx_counts$Sample == "SL"], 
       log10(sample_Tx_counts$Copy)[sample_Tx_counts$Sample == "2i_2d"])
t.test(log10(sample_Tx_counts$Copy)[sample_Tx_counts$Sample == "SL"], 
       log10(sample_Tx_counts$Copy)[sample_Tx_counts$Sample == "mTORi_1d"])
t.test(log10(sample_Tx_counts$Copy)[sample_Tx_counts$Sample == "SL"], 
       log10(sample_Tx_counts$Copy)[sample_Tx_counts$Sample == "mTORi_2d"])
