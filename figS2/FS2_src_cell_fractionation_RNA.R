...
# convert gene id
res1 <- biomaRt::select(org.Mm.eg.db,
                       keys = keys(org.Mm.eg.db::org.Mm.eg.db, keytype = "ENSEMBL"),
                       keytype = "ENSEMBL",
                       columns = c("ENTREZID", "SYMBOL"))
SLAM.table$gene_id <- res1$ENSEMBL[ match(SLAM.table$Name, res1$SYMBOL) ]
SLAM.table <- SLAM.table[!is.na(SLAM.table$gene_id) & SLAM.table$k..cpm.h. > 0, ] # 6534 genes

TimeLapse.table$gene_id <- res1$ENSEMBL[ match(TimeLapse.table$transcript, res1$SYMBOL) ]
