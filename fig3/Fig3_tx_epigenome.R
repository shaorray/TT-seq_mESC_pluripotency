...
# boxplot of directionality ~ chromatin state, copy, CpG
TU.intergenic.gr <- TU.gr[TU.gr$location == "intergenic"]
TU.intergenic.gr$Copy_SL <- dat6[, 1]

TU.intergenic.gr$TSS_state <- "Active"
TU.intergenic.gr$TSS_state[TU.intergenic.gr$chromHMM %in% 
                             c("BivalentChromatin", "Intergenic", "RepressedChromatin", "Heterochromatin")] <- "Inactive"

# chisq.test(table(TU.intergenic.gr$TSS_state, TU.intergenic.gr$Direction)) # p-value < 2.2e-16
g2.1 <- table(TU.intergenic.gr$TSS_state, TU.intergenic.gr$Direction) %>%
  reshape::melt() %>%
  ggplot(aes(x = Var.1, y = value, fill = Var.2)) +
  geom_bar(stat="identity") +
  geom_text(x = 1, y = 10000, label = "p < 2.2e-16") +
  xlab("") +
  ylab("Genes") +
  ggtitle("Intergenic") +
  scale_fill_manual(values = colors_9[1:2]) +
  theme_setting +
  theme(legend.position = "none")

g2.2 <- mcols(TU.intergenic.gr) %>% 
  as.data.frame() %>%
  ggplot(aes(x = TSS_state, y = log10(Copy_SL), fill = Direction)) +
  geom_boxplot() +
  xlab("") +
  scale_y_continuous(name = "Copy", breaks = (-1):2, labels = 10^((-1):2), limits = c(-1, 2)) +
  scale_fill_manual(values = colors_9[1:2]) +
  theme_setting +
  theme(legend.position = "none")

g2.3 <- mcols(TU.intergenic.gr) %>% 
  as.data.frame() %>%
  ggplot(aes(x = TSS_state, y = log10(CpG_counts), fill = Direction)) +
  geom_boxplot() +
  xlab("") +
  scale_y_continuous(name = "CpG counts", breaks = 0:2, labels = 10^(0:2), limits = c(0, 2.5)) +
  scale_fill_manual(values = colors_9[1:2]) +
  theme_setting +
  theme(legend.position = "none")
...
ChromHMM <- importRanges("../data/mESC_E14_12_dense.annotated.mm10.bed")
ChromHMM$name <- gsub("(.*?)_(.*)", "\\2", ChromHMM$name)
TU.gr <- importRanges("../data/TU_anno/mm10/TU_filter+LRNA_SL.gff3")
tx.tss.gr <- TU.gr[TU.gr$location == "protein_coding"]
...
rownames(dat5) <- sample_tx_counts_Rates$gene_id[sample_tx_counts_Rates$Sample == "SL"]

res1 <- biomaRt::select(EnsDb.Mmusculus.v79::EnsDb.Mmusculus.v79,
                       keys = as.character(rownames(dat5)),
                       keytype = "GENEID",
                       columns = "GENENAME")
res2 <- biomaRt::select(EnsDb.Mmusculus.v79::EnsDb.Mmusculus.v79,
                        keys = as.character(tx.tss.gr$gene_id),
                        keytype = "GENEID",
                        columns = "GENENAME")
dat5 <- dat5[res1$GENEID, ]
dat5 <- dat5[match(res2$GENENAME, res1$GENENAME), ]
tx.tss.gr <- tx.tss.gr[match(res2$GENEID, tx.tss.gr$gene_id)]
tx.tss.gr$Copy_SL <- dat5[, 1]

rm.idx <- !apply(dat5, 1, function(x) any(is.na(x)) | any(is.infinite(x)))
dat5 <- dat5[rm.idx, ]
tx.tss.gr <- tx.tss.gr[rm.idx]
tx.tss.gr$gene_id <- rownames(dat5)
...
as.tx.tss.gr <- tx.tss.gr; levels(strand(as.tx.tss.gr)) <- c("-", "+", "*")
tx.tss.gr$Direction <- "Unidirection"

bi_idx <- findOverlaps(as.tx.tss.gr, 
                       c(TU.gr[TU.gr$location == "uaRNA"], promoter.gr)) %>%
          countQueryHits() > 0
...
out <- pheatmap::pheatmap(out_dat, # 7038 genes
                          color = rev(viridis::inferno(100, begin = 0.2)),
                          border_color = NA, cellwidth = NA, cellheight = NA,
                          legend_breaks = (-1):3,
                          legend_labels = c("<0.1", 1, 10, 100, ">1000"),
                          ...
                          annotation_row = annotation_row,
                          annotation_colors = annotation_colors,
                          gaps_row = cumsum(table(cls[rownames(out_dat)])[c(4,3,2,1)]),
                          gaps_col = 3)
dev.off()

# boxplot of directionality ~ chromatin state, copy, CpG
tx.tss.gr$TSS_state <- "Inactive"
tx.tss.gr$TSS_state[tx.tss.gr$chromHMM == "ActivePromoter" | grepl("Enhancer", tx.tss.gr$chromHMM)] <- "Active"
# tx.tss.gr$TSS_state[tx.tss.gr$chromHMM == "BivalentChromatin"] <- "Bivalent"
# tx.tss.gr$TSS_state[grepl("Enhancer", tx.tss.gr$chromHMM)] <- "Enhancer"

# chisq.test(table(tx.tss.gr$TSS_state, tx.tss.gr$Direction)) # p-value < 2.2e-16
g2.4 <- table(tx.tss.gr$TSS_state, tx.tss.gr$Direction) %>% 
  reshape::melt() %>%
  ggplot(aes(x = Var.1, y = value, fill = Var.2)) +
  geom_bar(stat="identity") +
  geom_text(x = 1, y = 6400, label = "p < 2.2e-16") +
  ylim(c(0, 6500)) +
  xlab("") +
  ylab("Genes") +
  ggtitle("mRNA") +
...
g2.5 <- mcols(tx.tss.gr) %>% 
  as.data.frame() %>%
  ggplot(aes(x = TSS_state, y = Copy_SL, fill = Direction)) +
  geom_boxplot() +
  xlab("") +
  scale_y_continuous(name = "Copy", breaks = (-1):3, labels = 10^((-1):3), limits = c(-1, 3)) +
  scale_fill_manual(values = colors_9[1:2]) +
  theme_setting +
  theme(legend.position = "none")

# wilcox.test(log10(CpG_counts+1) ~ TSS_state, data = mcols(tx.tss.gr)) p-value = 0.8387
g2.6 <- mcols(tx.tss.gr) %>% 
  as.data.frame() %>%
  ggplot(aes(x = TSS_state, y = log10(CpG_counts), fill = Direction)) +
  geom_boxplot() +
  xlab("") +
...
ggsave(plot = egg::ggarrange(g2.4, g2.5, g2.6, g2.1, g2.2, g2.3, nrow = 1),
       filename = "Fig3_mRNA_intergenic_specific_boxplot.png", path = "figs",
       device = "png", width = 15, height = 3 )
