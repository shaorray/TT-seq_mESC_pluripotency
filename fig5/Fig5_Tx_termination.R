...
g3 <- data.frame(x = (read_thru_table$LRNA_SL + read_thru_table$LRNA_2i ) / 2e3,
  ...
  scale_x_continuous(name="Mean termination (kb)") +
  scale_y_continuous(name="2i Δtermination (kb)") +
...
g4 <- data.frame(x = (read_thru_table$LRNA_SL + read_thru_table$LRNA_mTORi ) / 2e3,
  ...
  scale_fill_viridis(option = "A", direction = -1) +
  scale_x_continuous(name="Mean termination (kb)") +
  scale_y_continuous(name="mTORi Δtermination (kb)") +
  theme(axis.title=element_text(size=16, face="bold"),
        legend.position = "none",
        panel.border = element_blank())
...
# elongation speed ~ termination length  ---------------------------------------------------
...
ggplot(elongation_table, aes(x = Speed_cls, y = read_through_SL / 1000, fill = Speed_cls)) +
  ...
  ylim(c(0, 15)) +
  xlab("") + ylab("termination distance SL (kb)") +
  theme_setting +
  theme(legend.position = "none")
...
if (T) {
  # termination indexing SL ~ 2i & SL ~ mTORi ------------------------------------------------------
...
if (T) {
  # estimated elongation speed changes ~ ttseq coverage and termination changes ---------------------------------------------------
...
# termination explanation with estimated elongation speed, gene length, tx length, etc.-----------------
...
dat_RT_bin <- dat_RT_bin %>% 
  mutate(feature = as.character(feature)) %>%
  mutate(feature = replace(feature, feature == "Est.RT_speed", "Est.Ter_velocity")) %>%
  mutate(feature = replace(feature, feature == "Est.GB_speed", "Est.GB_velocity")) %>%
  mutate(feature = replace(feature, feature == "RT_GC", "Ter_GC")) %>%
  mutate(feature = replace(feature, feature == "RT_DNAme", "Ter_DNAme")) %>%
...
ggplot(dat_RT_bin, aes(x = RT_class, y = feature)) +
  ...
  xlab("Termination distance (kb)") + ylab("") + ggtitle("Termination window density") +
  theme_minimal() +
...
ggsave(filename = "Fig5_Read_through_distance_feature_z_score.pdf",
       path = "../fig5/figs", device = "pdf", width = 5, height = 7) 
...
dat_variance_explained <- dat_variance_explained %>% 
  mutate(Feature = as.character(Feature)) %>%
  mutate(Feature = replace(Feature, Feature == "Est.RT_speed", "Est.Ter_velocity")) %>%
  mutate(Feature = replace(Feature, Feature == "Est.GB_speed", "Est.GB_velocity")) %>%
  mutate(Feature = replace(Feature, Feature == "RT_GC", "Ter_GC")) %>%
...
ggsave(filename = "Fig5_Read_through_distance_feature_explained.pdf",
       path = "../fig5/figs", device = "pdf", width = 5, height = 6) 

# (Fig EV6) non-linear explanation of termination distance ------------------------------------------------------------------
dat_test_gbm <- dat_test[dat_test$Read.through.length > 100,
                         !grepl("Control|MNase|H3K18ac|H3K18cr|H2BK20ac",
                            colnames(dat_test)) & !duplicated(colnames(dat_test))]

source("../fig5/F5_src_termiantion_gbm.R")

# (Fig EV6) termination sites epigenome feature coverage -----------------------------------------------------------------------
library(abind)
...
# ---------------------------------------------------------------------------------- #
# revision #3_4
# "dat_markers" is from "FigS2_RNA_turnover_comparison.R"
...
g1 <- ggplot(dat_markers2, aes(x = Type, y = Read_through / 1e3, fill = Type)) +
  ...
  xlab("\nPluripotency gene type") + ylab("termination distance (kb)") + 
  theme_setting +
...
g3 <- ggplot(dat_markers2, 
             aes(x = Read_through / 1e3, y = Speed,
...
if (F) {
  ...
  data.frame(x = (read_thru_table$LRNA_SL + TTseq.SL2i.ter.sites ) / 2e3,
    ...
    scale_fill_viridis(option = "A", direction = -1) +
    scale_x_continuous(name="Mean termination (kb)") +
    scale_y_continuous(name="SL2i Δtermination (kb)") +
    theme(axis.title=element_text(size=16, face="bold"),
          legend.position = "none",
          panel.border = element_blank())
