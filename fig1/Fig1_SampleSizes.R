...
g1 <- ggplot(cellCounts, aes(x = CellNum / 145, y = L_sf - F_sf, color = Color)) + 
  ...
  labs(color='Samples') +
  theme_setting
ggsave(g1, filename = "Fig1.Turn_over_cell_number.pdf", path = "figs",
       device = "pdf", width = 5, height = 4 )


tx_F_mat <- tx_F_mat[, !grepl("SL_rep3", colnames(tx_F_mat))]
tx_L_mat <- tx_L_mat[, !grepl("SL_rep3", colnames(tx_L_mat))]

colnames(tx_F_mat) <- gsub("(.*)\\_rep.*","\\1", colnames(tx_F_mat))
colnames(tx_L_mat) <- gsub("(.*)\\_rep.*","\\1",colnames(tx_L_mat))

tx_FRNA_sf <- tx_F_mat %>% meanSampleCounts %>% SizeFactorCal
tx_LRNA_sf <- tx_L_mat %>% meanSampleCounts %>% SizeFactorCal

sf_dat <- data.frame(Samples = names(log(tx_FRNA_sf)),
                     L_sf = log(tx_LRNA_sf),
                     F_sf = log(tx_FRNA_sf)) %>% 
  filter(Samples != "SL2i_2d")

g2 <- ggplot(sf_dat, aes(x = F_sf, y = L_sf, color = Samples, label = Samples)) + 
  geom_text(cex = 5, fontface = "bold") + 
  xlab("\nTotal RNA") +
  ylab("Nascent RNA\n") +
  xlim(c(-1, 1)) + 
...
ggsave(g2, filename = "Fig1.size_factors_distribution.pdf", path = "figs",
