...
# get genes intervals
gene.gr <- GenomicFeatures::genes(TxDb.Mmusculus.UCSC.mm10.knownGene)
...
fc_PE <- featureCounts(bam_files, annot.ext=gene.ann, isPairedEnd=TRUE)
...
fc_PE <- featureCounts(spliced_bam_files, annot.ext=gene.ann, isPairedEnd=TRUE)
...
spliced_ratio <- spliced_ratio[apply(spliced_ratio, 1, function(x) sum(x > 0) > 4), ] # remove unspliced gnes
